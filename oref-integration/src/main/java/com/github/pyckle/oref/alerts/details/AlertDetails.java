 ...
 * @param updateFlashType    if this alert is a flash or update, specifies the type. Null if it is not a flash
...
public record AlertDetails(
    ...
    public boolean isUpdateOrFlash() {
        return updateFlashType != null;
    }

    public boolean matchesArea(District d) {
        for (var loc : locationsHeb) {
            if (DistrictStore.COUNTRYWIDE_ALERT.contains(loc) || loc.equals(d.label_he())) {
                return true;
            }
        }
        return false;
